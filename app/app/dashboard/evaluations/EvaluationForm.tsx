...
export function EvaluationForm({ evaluationId, readOnly = false, calendarEventLink, onSaved, onCancel }: {
  ...
  const [reasonCancelledOther, setReasonCancelledOther] = useState('')
  ...
  const populateFromRow = useCallback((row: RawEvaluationRow) => {
    ...
    const reasonCancelledResolved = reverseResolve(REASONS_CANCELLED, row.reason_cancelled)
    setReasonCancelled(reasonCancelledResolved.slug)
    setReasonCancelledOther(reasonCancelledResolved.other)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!userId) return
    // Status is no longer a user-editable field -- recording an Evaluation
    // Outcome is what closes the evaluation (see payload below).
    if (evaluationOutcome === 'lost' && !reasonLost) { setError('Select a Reason Lost.'); return }
    if (evaluationOutcome === 'cancelled' && !reasonCancelled) { setError('Select a Reason Cancelled.'); return }
    ...
    const reasonLostLabel   = evaluationOutcome === 'lost'
      ...
      : null
    const reasonCancelledLabel = evaluationOutcome === 'cancelled'
      ? (reasonCancelled === 'other' ? `Other: ${reasonCancelledOther}` : (REASONS_CANCELLED.find(r => r.value === reasonCancelled)?.label ?? null))
      : null

    // Required-field set for "Evaluation Form Completed": Lead Generated By,
    // Lead Source, Contact Details, Evaluation Date and Time, Agent, and
    // Transaction Coordinator -- drives the pipeline step, which in turn
    // gates the Prepared status.
    ...
    const payload = {
      ...
      reason_cancelled:                 reasonCancelledLabel,
    ...
    const nowIso = new Date().toISOString()
    const { error: stepsErr } = await supabase.from('evaluation_pipeline_steps').insert(
    ...
    // A failure here (e.g. a DB constraint rejecting one of the seeded step
    // keys) previously failed completely silently -- the evaluation record
    // still got created, just with an empty, broken Pipeline tab and no
    // indication anything had gone wrong. Surface it loudly instead: the
    // record already exists at this point, so this can't be a clean abort,
    // but at least it's visible and points at what to fix.
    if (stepsErr) {
      setError(`Evaluation saved, but its pipeline steps failed to create: ${stepsErr.message}. Contact support -- the Pipeline tab will be empty until this is fixed.`)
      setSaving(false)
      return
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Section title="Evaluation Details">
        ...
        {/* Status is entirely gate-derived, never a manual choice -- no
            editable control here, ever, regardless of edit mode. */}
        <Field label="Evaluation Status" readOnly value={STATUS_LABELS[status] ?? status} />
